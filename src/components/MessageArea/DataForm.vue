...
<script>
  export default {
    ...
    methods: {
      handleSubmit() {
        this.error.nickname = this.formData.nickname.trim() ? "" : "请输入用户名";
        this.error.content = this.formData.content.trim() ? "" : "请输入评论内容";
        if (this.error.nickname || this.error.content) {
          //判断此时的错误信息是否有值，有值的情况下 返回 啥也不做
          return
        }
        //这时评论判断无问题 将进行下面的操作
        //  修改按钮状态（isSubmiting） 
        ...
        //  传递事件,完成后 回调进行下列操作
        this.$emit("submit", this.formData, (messageInfo) => {
          //  将输入框信息清除
          this.$showMessage({
            content: messageInfo,
            type: "success",
